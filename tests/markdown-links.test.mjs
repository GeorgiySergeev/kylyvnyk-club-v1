...
test('repository exposes a conventional root README', async () => {
  const readmePath = path.join(repoRoot, 'README.md')
  const misspelledReadmePath = path.join(repoRoot, 'READEME.md')

  assert.equal(await pathExists(readmePath), true)
  assert.equal(await pathExists(misspelledReadmePath), false)
})
  ...
  for (const filePath of markdownFiles) {
    ...
    for (const match of markdown.matchAll(localLinkPattern)) {
      ...
      const targetPath = path.resolve(
        fileDirectory,
        decodeURIComponent(rawTargetPath),
      )

      assert.ok(
        targetPath === repoRoot || targetPath.startsWith(`${repoRoot}/`),
        `${relativeFilePath} links outside the repository: ${href}`,
      )
